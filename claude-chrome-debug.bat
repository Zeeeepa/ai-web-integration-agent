...
set "LOG_FILE=%USERPROFILE%\.freeloader\claude_error.log"
set "DEBUG_MODE=1"
set "PORT=8000"
set "HOST=127.0.0.1"
set "BACKEND=ai-gateway"
...
echo ---------------------------------------- > "%LOG_FILE%"
echo Claude OpenAI API Deployment Log >> "%LOG_FILE%"
...
echo.
echo ===================================================
echo    Claude OpenAI API Endpoint Setup - Debug Mode
echo ===================================================
echo.
...
:: Import cookies from Chrome for claude.ai
...
:: Start the OpenAI API server with ai-gateway backend
...
start "Claude OpenAI API" cmd /c "echo Starting Claude OpenAI API server... & %PYTHON_CMD% freeloader_cli_main.py openai start --backend %BACKEND% --port %PORT% --host %HOST% --debug 2>> "%LOG_FILE%" || (echo Server crashed! Check %LOG_FILE% for details. & pause & exit /b 1) & echo Server running successfully! & pause"
...
echo curl -X POST http://%HOST%:%PORT%/v1/chat/completions -H "Content-Type: application/json" -d "{\"model\":\"claude-3-opus\",\"messages\":[{\"role\":\"user\",\"content\":\"Hello, Claude!\"}]}"
echo.
